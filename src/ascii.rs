...
use crate::schema::{AsciiFontVariant, AsciiReveal, AsciiRevealDirection, AsciiSource, ColorRgba};
    ...
    pub fn render_frame_pixmap(&self, frame_index: u32) -> Result<Pixmap> {
        ...
        for row in 0..self.source.grid.rows {
            for column in 0..self.source.grid.columns {
                ...
                self.paint_cell_glyph(
                    &mut pixmap,
                    origin_x,
                    origin_y,
                    character,
                    foreground,
                    self.source.font_variant,
                );
            }
        }
    ...
    fn paint_cell_glyph(
        ...
        for y in 0..cell_height {
            ...
            for x in 0..cell_width {
                ...
                if sample_variant_glyph(character, glyph_x, glyph_y, variant) {
                    blend_pixel(pixmap, origin_x + x, origin_y + y, color);
                }
            }
        }
...
fn sample_variant_glyph(character: u8, x: u32, y: u32, variant: AsciiFontVariant) -> bool {
...
fn sample_base_glyph(character: u8, x: u32, y: u32) -> bool {
    ...
    // Fixed deterministic atlas: each ASCII codepoint is represented as two
    // seven-segment hex digits inside an 8x8 glyph cell.
    if !(1..=5).contains(&y) {
        return false;
    }
