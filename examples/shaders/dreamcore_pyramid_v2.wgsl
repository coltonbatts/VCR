// Dreamcore Spinning Pyramid — Alpha Transparent
// Uses ShaderUniforms API for shader: path:
...
fn sdPyramid(p_input: vec3<f32>, h: f32) -> f32 {
    ...
    let s = max(-q.x, 0.0);
    let t_val = clamp((q.y - 0.5*p.z) / (m2 + 0.25), 0.0, 1.0);
    let a = q.x + s;
    let b = q.y - t_val*0.5;
    let c = q.z - t_val*h;
...
fn shade(uv: vec2<f32>, u: ShaderUniforms) -> vec4<f32> {
    let time = u.time * 0.033; // frame to ~seconds at 30fps
    let aspect = u.resolution.x / u.resolution.y;
    let p_ndc = (uv - 0.5) * vec2<f32>(aspect, 1.0);

    let ro = vec3<f32>(0.0, 0.4, -1.8);
    let rd = normalize(vec3<f32>(p_ndc * 0.8, 1.0));
    ...
    if (hit) {
        ...
        let color_top = vec3<f32>(1.0, 0.2, 0.9);
        let color_bottom = vec3<f32>(0.2, 0.9, 1.0);
