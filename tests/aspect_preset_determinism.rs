...
fn run_capture(cwd: &Path, aspect: &str) -> PathBuf {
    let output = Command::new(env!("CARGO_BIN_EXE_vcr"))
        ...
        .output()
        .expect("ascii capture should execute");
    assert!(
        output.status.success(),
        "capture should succeed for aspect={aspect}. stdout={} stderr={}",
    ...
    let mov_line = stdout
        ...
        .expect("capture should print mov output path");
...
#[test]
fn ascii_capture_is_byte_deterministic_for_all_aspect_presets() {
    ...
    for aspect in ["cinema", "social", "phone"] {
        ...
        let mov_a = run_capture(run_a.path(), aspect);
        let mov_b = run_capture(run_b.path(), aspect);
