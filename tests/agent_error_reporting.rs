...
"#;
    ...
    assert!(
        stderr.contains("\"error_type\""),
        "Should contain error_type field"
    );
    assert!(
    ...
    assert!(
        stderr.contains("\"suggested_fix\""),
        "Should contain suggested_fix"
    );
    assert!(
        stderr.contains("duration"),
        "Should mention missing duration field"
    );
}
...
"#;
    ...
    assert!(
        stderr.contains("\"error_type\": \"lint\""),
        "Should be a lint error"
    );
    assert!(
        stderr.contains("unreachable"),
        "Should mention unreachable layer"
    );
    assert!(
        stderr.contains("\"suggested_fix\""),
    ...
    assert!(
        stderr.contains("start_time") || stderr.contains("opacity"),
        "Fix should mention timing or opacity"
    );
}
...
"#;
    ...
    assert!(
        stderr.starts_with("vcr "),
        "Should start with 'vcr' command name"
    );
    assert!(
        !stderr.contains("\"error_type\""),
        "Should NOT be JSON in normal mode"
    );
    assert!(
        !stderr.contains("\"suggested_fix\""),
...
"#;
    ...
    assert!(
        stderr.contains("\"error_type\": \"usage\""),
        "Should be a usage error"
    );
    assert!(
        stderr.contains("speed") || stderr.contains("param"),
        "Should mention parameter"
    );
}
...
fn vcr_binary() -> PathBuf {
    ...
    let debug = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("target/debug/vcr");
    if debug.exists() {
        return debug;
    }

    // Try release build
    let release = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("target/release/vcr");
    if release.exists() {
        return release;
    }
